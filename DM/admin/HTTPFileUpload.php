...
else if(isset($_FILES))
{
    ...
    //Validate extension
    $isValidExtension = true;//($ext == 'jpg' || $ext == 'jpeg' || $ext == 'png' || $ext == 'gif');     
    if (!$isValidExtension) {
        $response->success = false;
        $response->message = '';
    }
    else
    {
        //Move file to app
        $uid = uniqid("", true).".".$ext;
        if (move_uploaded_file ($temp_file_name, "uploaded_files//".$uid)) {
            
            /*
            //create thumb
            $filename = "uploaded_files//".$uid;            
            $thumb = "uploaded_files//thumb_".$uid;
