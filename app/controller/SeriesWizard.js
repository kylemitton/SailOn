...
Ext.define('DynaMightMobile.controller.SeriesWizard', {
    ...
    config: {
        refs: {
            ...
            optionValueTxt: 'textfield#optionValueTxt',
            optionTxt: 'textfield#optionTxt',
            raceDetailsEdit: 'formpanel#raceDetailsEdit'
        },
        ...
        control: {
            ...
            "button#deleteDivisionBtn": {
                tap: 'onDeleteDivisionBtnTap'
            },
            "formpanel#raceAddFrm": {
                initialize: 'onRaceAddFrmInitialize'
            },
            "button#backBtn1": {
                tap: 'onBackBtn1Tap'
            }
    ...
    onNextButtonTap: function(button, e, eOpts) {
        ...
        if(selectedIndex === 0){
            ...
            idx = 2;
            /*TODOLPOPUP
            var optionTxt = me.getDivisionDetailsEdit().down('#optionTxt').getValue();
            if(optionTxt == 'new' || optionTxt == 'edit'){
                idx=1;
            }*/
        }
    ...
    onNewRaceButtonTap: function(button, e, eOpts) {
        ...
        if(number){
             ...
             for (var i = 0; i < number; i++) {
                 ...
                 //store.add(records);
                 //me.getRacesList().setStore(store);
                 //me.getRacesList().refresh();
                 //store.add(records);
    ...
    onDelDivisionButtonTap: function(button, e, eOpts) {
        ...
        if (selected.length > 0) {
            ...
            this.getDivisionsList().getStore().add(selected);
            this.getDivisionsList().refresh();
        }
    },

    onNewDivisionButtonTap: function(button, e, eOpts) {
        var me = this;
                var pop = me.getDivisionDetailsEdit(),
                wizard = me.getSeriesWizard(),
                selectedIndex = wizard.items.indexOf(wizard._activeItem),
                idx = 1;

        me.getDivisionDetailsEdit().reset();
        //me.getSeriesWizard().setActiveItem(5);

        /*TODOLPOPUP
                pop.showBy(button);
                pop.setCentered(true);
                pop.setWidth('50%');
                pop.setHeight('75%');
                pop.setModal(true);
         */
                me.getOptionTxt().setValue("new");
    ...
    onUpdateDivisionButtonTap: function(button, e, eOpts) {
        ...
        //me.getSeriesWizard().setActiveItem(2);
        rec = selected.get(0);
        me.getDivisionDetailsEdit().setRecord(rec);
        this.getDivisionDetailsEdit().setRecord(rec);

        /*TODOLPOPUPvar pop = me.getDivisionDetailsEdit();
        ...
        pop.setModal(true);
        */
        this.getOptionTxt().setValue('edit');

        //pop.getLayoutConfig().setAlign('stretch');
        me.getSeriesWizard().setActiveItem(3);
    },
    ...
    onSelectfieldChange: function(selectfield, newValue, oldValue, eOpts) {
        if(newValue == 'CBH'){
            var handicapValueTxt = this.getDivisionDetails().down('#handicapValueTxt');

            if(handicapValueTxt != null){
               handicapValueTxt.setHidden(false);
            }
        }
        else{
           //this.getDivisionDetails().down('#handicapValueTxt').setHidden(true);
            var handicapvalue = this.getDivisionDetails().down('#handicapvalue');

            if(handicapvalue != null){
                handicapvalue.setHidden(true);
            }
        }
    },
    ...
    onSaveDivisionBtnTap: function(button, e, eOpts) {
        ...
        if(me.getOptionTxt().getValue() == "new"){
            me.newDevision();
        }else{
            me.updateDevision();
        }

        //TODOLPOPUP me.getDivisionDetailsEdit().hide();
        me.getSeriesWizard().animateActiveItem(2, {type: 'slide', direction: 'right'});
        me.getOptionTxt().setValue(null);
    },
    ...
    onDeleteDivisionBtnTap: function(button, e, eOpts) {
        var rec, id,
            list = this.getRaceDivisionsList(),
            selected = this.getRaceDivisionsList().selected,
            frmValues = this.getDivisionDetailsEdit().getValues();
        ...
        Ext.Msg.confirm("Confirmation", "Are you sure you want to remove '"+ rec.get('name') +"'?",
            function(btn){
                if(btn === 'yes'){
                    ...
                    AJAXCommand({
                        ...
                        callback: function(status, message, obj, scope) {
                            ...
                            {
                                Ext.Msg.show({
                                    title: 'Error deleting Division',//T('errLogIn'),
                                    msg: 'Error.',//T(''),
                                    buttons: Ext.Msg.OK,
                                    icon: Ext.Msg.ERROR,
                                    modal: true
    ...
    onRaceAddFrmInitialize: function(component, eOpts) {
        //component.show();
        //component.hide();

        /*var me =  this;
            me.getMyfieldset8.setHidden(true);
            me.getRaceCt().setHidden(true);
        */
        //newTxt


        //var me =  this;
        //me.getNewTxt().setHidden(true);

        //me.getStartFrm().setHidden(true);
        //me.getPatternFrm().setHidden(true);
    },

    onBackBtn1Tap: function(button, e, eOpts) {
        this.getController('Navigation').onNavigate();
    },
    ...
    updateDevision: function() {
        ...
        //debugger;
                ...
                rec.set('groupname', frmValues.groupname);
                ...
                AJAXCommand({
                    params: {
                        divisionid: id,
                        name: frmValues.name,
                        groupname: frmValues.groupname,
                        handicaptype: frmValues.handicaptype,
                        handicapvalue: frmValues.handicapvalue,
    ...
    newDevision: function() {
        var rec, id, frm,
            me = this,
            list = me.getRaceDivisionsList(),
            divisionEdit = me.getDivisionDetailsEdit(),
            selected = me.getDivisionsList().selected;

        var option = divisionEdit.down('#optionTxt');
        ...
        if(option.getValue() != null && option.getValue() != ''){
            ...
            frm = divisionEdit.getValues();

            name= frm.name;
            groupname= frm.groupname;
            handicaptype= frm.handicaptype;
            handicapvalue= frm.handicapvalue;
